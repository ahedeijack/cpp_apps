...
void graficos::lineaDDA(int x1, int y1, int x2, int y2, QPainter *canvas)
{
    ...
    canvas->setPen(QColor(166, 0, 255));// RECIBE EL COLOR
...
void graficos::dragon(QPainter *canvas)
{
    int i;
    int Paso = 4096;
    int signo = -1;
    ...
    canvas->setPen(QColor(166, 0, 255));
    ...
    for( i = 1; i <= 11; i++)
    {
        generarDragon(Paso, signo, EjeX, EjeY, canvas); //generarDragon(Canvas);
        Paso /= 2; // Paso = Paso / 2;
    }
}

void graficos::generarDragon(int Paso, int signo, int EjeX[4098], int EjeY[4098], QPainter *canvas)
{
    int j, dx, dy;
    j = Paso / 2;
    canvas->setPen(QColor(166, 0, 255));//qrand()%
                             // signo *= -1;
...
void graficos::dragonArabesco(QPainter *canvas)
{
    int i;
    int Paso = 4096;
    int signo = -1;
    ...
    for( i = 1; i <= 11; i++)
    {
        generarDragonArabesco(Paso, signo, EjeX, EjeY, canvas); //generarDragon(Canvas);
        Paso /= 2; // Paso = Paso / 2;
    }
}

void graficos::generarDragonArabesco(int Paso, int signo, int EjeX[], int EjeY[], QPainter *canvas)
{
    int j, dx, dy;
    j = Paso / 2;
    canvas->setPen(QColor(166, 0, 255));//qrand()%
...
void graficos::curvaHenon(QPainter *canvas)
{
    ...
    fuente.setPointSize(fuente.pointSize() * 2);
    //Canvas.setFont(fuente);
    generarCurvaHenon(maxX, maxY, EscalaX, EscalaY, DespX, DespY, canvas);


}

void graficos::generarCurvaHenon(int maxX, int maxY, double EscalaX, double EscalaY, double DespX, double DespY, QPainter *canvas)
{
    int i, PosX, PosY;
...
void graficos::malthus1(QPainter *canvas)
{
    int maxX = canvas->window().width();
    int maxY = canvas->window().height();
    int MaxGen = 0x00FF;
...
void graficos::malthus2(QPainter *canvas)
{
    int maxX = canvas->window().width();
    int maxY = canvas->window().height();
...
void graficos::mandelbrot(QPainter *canvas)
{
